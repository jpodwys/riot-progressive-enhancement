...
var riot = require('riot');
module.exports = riot.tag2('entry-list', '<div each="{opts.entries}"> <a href="/entry/{id}">{this.parent.formatDate(date)}</a> <span if="{!isPublic}">Private</span> <span if="{isPublic}">Public</span> <p>{text}</p> </div>', '', '', function(opts) {
    var self = this;
...
var riot = require('riot');
module.exports = riot.tag2('new-entry', '<form method="post" action="/entry" onsubmit="{create}"> <input name="date" value="{opts.entry.date}"> <textarea name="text"></textarea> Is Public: <input type="checkbox" name="isPublic" __checked="{opts.entry.isPublic}"> <input type="submit"> </form>', '', '', function(opts) {
    var self = this;
    opts.entry = {
